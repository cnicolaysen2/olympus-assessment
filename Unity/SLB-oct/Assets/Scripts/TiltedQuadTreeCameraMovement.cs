...
namespace SLB
{
    ...
    public class TiltedQuadTreeCameraMovement : MonoBehaviour
    {
        [SerializeField]
        [Range(1, 100)]
        ...
        private bool _tiltStarted;
        private float _cameraRotationSpeed = 1;

        [SerializeField]
        private float limitTiltUp = 35;
        [SerializeField]
        private float limitTiltDown = 86;
        ...
        void TiltCamera()
        {
            ...
            Transform camTr = _referenceCamera.transform;

            if (Math.Abs(xMove) < Math.Abs(yMove))
            {
                //when camera tilted to horizont yMove is negative; otherwise it's positive
                Vector3 upLimitVector =  Quaternion.Euler(limitTiltUp, camTr.eulerAngles.y, camTr.eulerAngles.z) * Vector3.forward;
                Vector3 downLimitVector =  Quaternion.Euler(limitTiltDown, camTr.eulerAngles.y, camTr.eulerAngles.z) * Vector3.forward;
                Vector3 cameraVector =  camTr.transform.forward;


                //if ( yMove < 0 && Vector3.Cross(cameraVector, upLimitVector).x < 0) TODO still buged
                //    camTr.RotateAround(this.cameraRotationCenter.position, camTr.right, yMove);
                
                //if ( yMove > 0 && Vector3.Cross(downLimitVector, cameraVector).x < 0 )
                    camTr.RotateAround(this.cameraRotationCenter.position, camTr.right, yMove);

            }
            else
            {
                camTr.RotateAround(this.cameraRotationCenter.position, this.cameraRotationCenter.up, xMove);
            }
        ...
        private void OnDrawGizmos()
        {
            ...
            //Gizmos.DrawLine(camTr.position, camTr.position + Quaternion.Euler(limitTiltDown, camTr.eulerAngles.y, camTr.eulerAngles.z) * Vector3.forward * 5);
            //Gizmos.color = Color.magenta;
            //Gizmos.DrawLine(camTr.position, camTr.position + Quaternion.Euler(limitTiltUp, camTr.eulerAngles.y, camTr.eulerAngles.z) * Vector3.forward * 5);
            //Vector3 cameraVector = camTr.position + camTr.transform.forward * 5;
            //Gizmos.color = new Color(0,1,0,0.4f);
            //Gizmos.DrawLine(camTr.position, cameraVector);
            //Gizmos.DrawSphere(this.cameraRotationCenter.position, (this.cameraRotationCenter.position - camTr.position).magnitude);
            //Gizmos.color = new Color(1, 0, 0);
            //Gizmos.DrawLine(camTr.position, camTr.position + camTr.right * 5);
            //Gizmos.DrawSphere(camTr.position, 0.3f);
        }
            ...
            if (_shouldDrag == true)
            {
                ...
                if (Mathf.Abs(changeFromPreviousPosition.x) > 0.0f || Mathf.Abs(changeFromPreviousPosition.y) > 0.0f)
                {
                    ...
                    if (Mathf.Abs(offset.x) > 0.0f || Mathf.Abs(offset.z) > 0.0f)
                    {
                        if (null != _mapManager)
                        {
                            float factor = _panSpeed * Conversions.GetTileScaleInMeters(0, _mapManager.AbsoluteZoom) / _mapManager.UnityTileSize;
