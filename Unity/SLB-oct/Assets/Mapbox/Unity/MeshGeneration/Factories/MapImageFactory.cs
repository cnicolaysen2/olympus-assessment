	...
	public class MapImageFactory : AbstractTileFactory
	{
		...
		protected virtual void OnDestroy()
		{
			...
			if (DataFetcher != null)
			{
                ...
                DataFetcher.LayerDataRecieved -= OnImageLayerRecieved;
                DataFetcher.LayerRawDataRecieved -= OnImageRawLayerRecieved;
        ...
        private void OnImageLayerRecieved(UnityTile tile, RasterTile rasterTile, Layers layer)
        {
            if (tile != null)
            {
                if (tile.RasterDataState != TilePropertyState.Unregistered)
                {
                    _tilesWaitingResponse.Remove(tile);
                    tile.SetRasterData(rasterTile.Data, layer, _properties.rasterOptions.useMipMap, _properties.rasterOptions.useCompression);
                }
            }
        }
		...
		protected override void OnRegistered(UnityTile tile)
		{
			if (_properties.sourceType == ImagerySourceType.None)
			{
                ...
                {
                    tile.SetRasterData(null, layer: layer);
                }
			...
			else
			{
				...
				DataFetcher.FetchData(parameters, false);
			}
		}
