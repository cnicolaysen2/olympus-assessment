...
namespace Mapbox.Unity.MeshGeneration.Data
{
    ...
    public class UnityTile : MonoBehaviour
	{
		...
		public void SetHeightData(byte[] data, float heightMultiplier = 1f, bool useRelative = false, bool addCollider = false, float displacement = 0)
		{
			if (HeightDataState != TilePropertyState.Unregistered)
			{
				...
				for (int xx = 0; xx < 256; ++xx)
				{
					for (int yy = 0; yy < 256; ++yy)
					{
                        ...
                        if (r + g + b == 0)
                        {
                            _heightData[xx * 256 + yy] = 0;
                        }
                        else
                        {      
                            _heightData[xx * 256 + yy] = relativeScale * heightMultiplier * (Conversions.GetAbsoluteHeightFromColor(r, g, b) + displacement * 100);
                        }                        
                    }
				}
        ...
        private Texture2D _elevationRasterData;
        private Texture2D _porosityRasterData;
        private Texture2D _thicknessRasterData;
        private Texture2D _TOCRasterData;
        private Texture2D _VShaleRasterData;

        private void OnDisable()
        {
            foreach (Layers layer in (Layers[])Enum.GetValues(typeof(Layers)))
            {
                MeshRenderer.material.SetTexture("_" + layer.ToString() + "_Texture", null);
            }
        }

        public void SetRasterData(byte[] data, Layers layer, bool useMipMap = true, bool useCompression = false)
        {
            // Don't leak the texture, just reuse it.
            if (RasterDataState != TilePropertyState.Unregistered)
            {
                ...
                switch (layer)
                {
                    case Layers.Elevation:
                        if (_elevationRasterData == null)
                        {
                            _elevationRasterData = new Texture2D(0, 0, TextureFormat.RGB24, useMipMap);
                            _elevationRasterData.wrapMode = TextureWrapMode.Clamp;
                        }

                        _elevationRasterData.LoadImage(data);

                        if (useCompression)
                        {
                            // High quality = true seems to decrease image quality?
                            _elevationRasterData.Compress(false);
                        }

                        MeshRenderer.material.SetTexture("_" + layer.ToString() + "_Texture", _elevationRasterData);

                        break;
                    case Layers.Porosity:
                        if (_porosityRasterData == null)
                        {
                            _porosityRasterData = new Texture2D(0, 0, TextureFormat.RGBA32, useMipMap);
                            _porosityRasterData.wrapMode = TextureWrapMode.Clamp;
                        }

                        _porosityRasterData.LoadRawTextureData(data);

                        if (useCompression)
                        {
                            // High quality = true seems to decrease image quality?
                            _porosityRasterData.Compress(false);
                        }

                        MeshRenderer.material.SetTexture("_" + layer.ToString() + "_Texture", _porosityRasterData);

                        break;
                    case Layers.Thickness:
                        if (_thicknessRasterData == null)
                        {
                            _thicknessRasterData = new Texture2D(0, 0, TextureFormat.RGB24, useMipMap);
                            _thicknessRasterData.wrapMode = TextureWrapMode.Clamp;
                        }

                        _thicknessRasterData.LoadImage(data);

                        if (useCompression)
                        {
                            // High quality = true seems to decrease image quality?
                            _thicknessRasterData.Compress(false);
                        }

                        MeshRenderer.material.SetTexture("_" + layer.ToString() + "_Texture", _thicknessRasterData);

                        break;
                    case Layers.TOC:
                        if (_TOCRasterData == null)
                        {
                            _TOCRasterData = new Texture2D(0, 0, TextureFormat.RGB24, useMipMap);
                            _TOCRasterData.wrapMode = TextureWrapMode.Clamp;
                        }

                        _TOCRasterData.LoadImage(data);

                        if (useCompression)
                        {
                            // High quality = true seems to decrease image quality?
                            _TOCRasterData.Compress(false);
                        }

                        MeshRenderer.material.SetTexture("_" + layer.ToString() + "_Texture", _TOCRasterData);

                        break;
                    case Layers.VShale:
                        if (_VShaleRasterData == null)
                        {

                            _VShaleRasterData = new Texture2D(256, 256, TextureFormat.RFloat, false);
        
                          
                            var example = _VShaleRasterData.GetPixel(10, 15);
                           // _VShaleRasterData.wrapMode = TextureWrapMode.Clamp;
                        }

                        _VShaleRasterData.LoadImage(data);

                        //float[] floats = _VShaleRasterData.GetPixels().Select(i => i.r).ToArray();
                        //int[] buckets = HistogrammHelper.Bucketize(floats, 100, float.MaxValue);
                        //Debug.Log($" floats {floats.Max()} buckets {buckets.Max()}");

                        MeshRenderer.material.SetTexture("_" + layer.ToString() + "_Texture", _VShaleRasterData);
